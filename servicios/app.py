...
# ============================================
# CONFIGURACION SUBIDAS DE ARCHIVOS
# ============================================
...
def exito():
    ...
    # Guardar pago si viene de Mercado Pago y está aprobado
    if payment_id and metodo == 'mercadopago' and status == 'approved':
        try:
            ...
            with conn.cursor() as cursor:
                ...
                cursor.execute("SELECT id FROM pagos WHERE paypal_order_id = %s", (payment_id,))
                if not cursor.fetchone() and producto_id > 0:
...
# ============================================
# API - PERFIL (CORREGIDO PARA FOTOS)
# ============================================
...
def api_actualizar_perfil():
    ...
    # Procesar archivo de foto si viene
    if 'foto' in request.files:
        file = request.files['foto']
        if file and file.filename != '' and allowed_file(file.filename):
            ext = file.filename.rsplit('.', 1)[1].lower()
            foto_filename = f"user_{session['usuario_id']}_{uuid.uuid4().hex[:8]}.{ext}"
            ...
            try:
                with conn.cursor() as cursor:
                    ...
                    old = cursor.fetchone()
                    if old and old.get('foto'):
                        old_path = os.path.join(app.config['UPLOAD_FOLDER'], old['foto'])
    ...
    try:
        with conn.cursor() as cursor:
            if foto_filename:
                cursor.execute(
                    "UPDATE usuarios SET nombre = %s, email = %s, usuario = %s, foto = %s WHERE id = %s",
                    (nombre, email, usuario, foto_filename, session['usuario_id'])
                )
                session['usuario_foto'] = foto_filename
            else:
                cursor.execute(
            ...
            return jsonify({'success': True, 'foto': foto_filename})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})
    finally:
        conn.close()
